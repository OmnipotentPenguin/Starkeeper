...
public class ArticleServiceTests {
	...
	public void deleteArticleTest() {
		...
		assertTrue("Article has not been updated", service.deleteArticle(1L).equals("Article Deleted"));
	...
	@Test
	public void getArticlesByNameTest() {
		
		List<Article> articles = new ArrayList<Article>();
		articles.add(new Article("article_name", "www.url.com"));
		Mockito.when(service.getArticlesByName("article")).thenReturn(articles);
		
		assertTrue("Returned no articles", service.getArticlesByName("article").size() > 0);

		Mockito.verify(service).getArticlesByName("article");
		
	}
	
	@Test
	public void getArticlesByFavouriteTest() {
		
		List<Article> articles = new ArrayList<Article>();
		articles.add(new Article("article_name", "www.url.com"));
		Mockito.when(service.getArticlesByFavourite(true)).thenReturn(articles);
		
		assertTrue("Returned no articles", service.getArticlesByFavourite(true).size() > 0);

		Mockito.verify(service).getArticlesByFavourite(true);
	}
